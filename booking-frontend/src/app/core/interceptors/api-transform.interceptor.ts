...
export const apiTransformInterceptor: HttpInterceptorFn = (
  req,
  next,
): Observable<HttpEvent<unknown>> => {
  // Auth and admin endpoints use camelCase on the wire (per contract.yaml §10 naming_convention).
  // Only convert other requests: camelCase → snake_case
  const skipTransform = req.url.includes('/auth/') || req.url.includes('/admin/');
  const transformedReq = req.clone({
    body:
      !skipTransform && req.body && typeof req.body === 'object' && !(req.body instanceof FormData)
