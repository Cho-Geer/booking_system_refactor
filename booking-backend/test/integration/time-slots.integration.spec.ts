...
import { CreateTimeSlotDto, UpdateTimeSlotDto } from '@/modules/time-slots/dto/time-slot.dto';
import { PrismaService } from '@/common/database/prisma.service';
import { createTestModule, TestModule } from '../helpers/create-test-module';
import { createTestService, createTestTimeSlot } from '../fixtures/database.fixture';
...
describe('[Integration] TimeSlotsService', () => {
  ...
  describe('getAvailableSlots', () => {
    ...
    it('should return available slots with correct shape', async () => {
      ...
      const result = await service.getAvailableSlots(svc.id, startDate, endDate);
    ...
    it('should throw NotFoundException when service does not exist', async () => {
      await expect(service.getAvailableSlots('nonexistent-id', startDate, endDate)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should handle multi-day date range', async () => {
      ...
      const result = await service.getAvailableSlots(svc.id, multiDayStart, multiDayEnd);
    ...
    it('should set available=false for slots exceeding overtime limit', async () => {
      ...
      const result = await service.getAvailableSlots(svc.id, startDate, endDate, 45);

      const slotsWithOvertime = result.filter((s) => s.maxOvertimeMinutes !== undefined);
