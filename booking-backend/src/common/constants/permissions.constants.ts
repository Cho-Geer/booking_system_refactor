...
export const PERMISSIONS_MATRIX = {
  ...
  roles: {
    CUSTOMER: {
      name: 'Customer',
      description: 'Customer who can book appointments and manage their profile',
      permissions: [
        'user:read:own',
        'user:update:own',
        'service:read',
  ...
  endpointPermissions: {
    ...
    'PATCH /v1/bookings/:id/cancel': ['booking:cancel:own', 'booking:cancel:any'],
...
export function hasPermission(userRole: string, permission: string): boolean {
  ...
  for (const [higherPermission, impliedPermissions] of Object.entries(hierarchy)) {
    if (impliedPermissions.includes(permission) && rolePermissions.includes(higherPermission)) {
      return true;
    }
  }
  ...
  for (const inheritedRole of inheritedRoles) {
    ...
    const inheritedPermissions = PERMISSIONS_MATRIX.roles[inheritedRoleKey]?.permissions || [];
 ...
 */
export const DEFAULT_USER_PERMISSIONS = PERMISSIONS_MATRIX.roles.CUSTOMER.permissions;
