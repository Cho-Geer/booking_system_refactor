  ...
  constructor(@Optional() @Inject(ClsService) private readonly cls?: ClsService) {}
    ...
    if (exception instanceof HttpException) {
      ...
      if (Array.isArray(rawMessage)) {
        ...
        message = typeof rawMessage === 'string' ? rawMessage : (rawMessage as string) || message;
        error = typeof exceptionResponse === 'string' ? exceptionResponse : message;
      }
    ...
    const requestId = this.cls?.get('requestId') ?? `req-${crypto.randomUUID()}`;
